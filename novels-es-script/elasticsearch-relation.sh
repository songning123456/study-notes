...
delIndex()
{
	curl -H "Content-Type: application/json" -XDELETE -i -s "$1/relation_index" > /dev/null
	echo "delete index relation_index successfully !"

}

createIndex()
{
  curl -H "Content-Type: application/json" -XPUT -i -s $1/relation_index -d '
	{
	  "settings": {
	...
	echo " create relation_index successfully !"
